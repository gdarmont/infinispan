...
infinispan.client.hotrod.executor_factory = org.infinispan.client.hotrod.impl.executors.DefaultExecutorFactory
...
All keys can be retrieved from the remote cache (whether it's local, replicated, or
distributed) by using +keySet()+ method. If the remote cache is a distributed cache,
the server will perform a distributed stream operation to retrieve all keys from
clustered nodes, and return all keys to the client.  Please use this method with
care if there are large number of keys.
...
include::chapter-78-Scripting.adoc[]
